...
public abstract class RendererState<T, V> implements SceneRenderer<T, V>, ILimitResettable {
  ...
  public RendererState(DoomMain<T, V> DOOM) {
    ...
    @SuppressWarnings("unchecked") final TextureManager<T> tm =
        (TextureManager<T>) new SimpleTextureManager(DOOM);
    this.TexMan = tm;
  ...
  public void SetupFrame(player_t player) {
    ...
    if (setblocks == 11) {
      tempCentery =
          (view.height / 2) + (int) (view.lookdir * DOOM.vs.getScreenMul() * setblocks) / 11;
    } else {
      tempCentery =
          (view.height / 2) + (int) (view.lookdir * DOOM.vs.getScreenMul() * setblocks) / 10;
    }
  ...
  public void ExecuteSetViewSize() {
    ...
    if (setblocks == 11) {
      ...
      view.height =
          (short) ((setblocks * (DOOM.vs.getScreenHeight() - DOOM.statusBar.getHeight()) / 10) &
              ~7);
    }
    ...
    MyThings.setPspriteScale(
        (int) (FRACUNIT * (DOOM.vs.getScreenMul() * view.width) / DOOM.vs.getScreenWidth()));
    MyThings.setPspriteIscale(
        (int) (FRACUNIT * (DOOM.vs.getScreenWidth() / (view.width * DOOM.vs.getScreenMul()))));
    vp_vars.setSkyScale(
        (int) (FRACUNIT * (DOOM.vs.getScreenWidth() / (view.width * DOOM.vs.getScreenMul()))));

    view.BOBADJUST = this.DOOM.vs.getSafeScaling() << 15;
    view.WEAPONADJUST =
        (int) ((DOOM.vs.getScreenWidth() / (2 * DOOM.vs.getScreenMul())) * FRACUNIT);
    ...
    for (int i = 0; i < colormaps.lightLevels(); i++) {
      startmap =
          ((colormaps.lightLevels() - colormaps.lightBright() - i) * 2) * colormaps.numColorMaps() /
              colormaps.lightLevels();
  ...
  public void FillBackScreen() {
    final boolean scaleSetting =
        Engine.getConfig().equals(Settings.scale_screen_tiles, Boolean.TRUE);
    flat_t src;
    DoomScreen dest;
    int x;
    int y;
     ...
     */
    {
      this.backScreenRect.setBounds(0, 0, DOOM.vs.getScreenWidth(),
          DOOM.vs.getScreenHeight() - DOOM.statusBar.getHeight());
      this.tilePatchRect.setBounds(0, 0, 64, 64);
      V block = DOOM.graphicSystem.convertPalettedBlock(src.data);
      if (scaleSetting) {
        block = DOOM.graphicSystem.ScaleBlock(block, DOOM.vs, tilePatchRect.width,
            tilePatchRect.height);
    ...
    for (x = 0; x < view.scaledwidth; x += stepX) {
      DOOM.graphicSystem.DrawPatchScaled(BG, patch, DOOM.vs, view.windowx + x, view.windowy - stepY,
          scaleFlags);
    }

    patch = DOOM.wadLoader.CachePatchName("BRDR_B", PU_CACHE);
    for (x = 0; x < view.scaledwidth; x += stepX) {
      DOOM.graphicSystem.DrawPatchScaled(BG, patch, DOOM.vs, view.windowx + x,
          view.windowy + view.height, scaleFlags);
    }

    patch = DOOM.wadLoader.CachePatchName("BRDR_L", PU_CACHE);
    for (y = 0; y < view.height; y += stepY) {
      DOOM.graphicSystem.DrawPatchScaled(BG, patch, DOOM.vs, view.windowx - stepX, view.windowy + y,
          scaleFlags);
    }

    patch = DOOM.wadLoader.CachePatchName("BRDR_R", PU_CACHE);
    for (y = 0; y < view.height; y += stepY) {
      DOOM.graphicSystem.DrawPatchScaled(BG, patch, DOOM.vs, view.windowx + view.scaledwidth,
          view.windowy + y, scaleFlags);
    }

    // Draw beveled edge. Top-left
    patch = DOOM.wadLoader.CachePatchName("BRDR_TL", PU_CACHE);
    DOOM.graphicSystem.DrawPatchScaled(BG, patch, DOOM.vs, view.windowx - stepX,
        view.windowy - stepY, scaleFlags);
    ...
    DOOM.graphicSystem.DrawPatchScaled(BG, patch, DOOM.vs, view.windowx + view.width,
        view.windowy + view.height, scaleFlags);
  }
  ...
  protected void InitTextureMapping() {
    ...
    for (x = 0; x <= view.width; x++) {
      ...
      view.xtoviewangle[x] = addAngles(((long) i << ANGLETOFINESHIFT), -ANG90);
    }
  ...
  protected void R_InitTranMap(int progress) {
    ...
    { // Compose a default transparent filter map based on PLAYPAL.
      ...
      if (MenuMisc.WriteFile("tranmap.dat", main_tranmap,
          main_tranmap.length)) {
        System.out.print(
            "TRANMAP.DAT saved to disk for your convenience! Next time will be faster.\n");
      }
    }
  ...
  protected final class BSP extends BSPVars {
    ...
    private void AddLine(seg_t line) {
      ...
      if (backsector.ceilingheight <= frontsector.floorheight
        ...
        if (DEBUG) {
          System.out.println(
              "Entering ClipSolidWallSegment Closed door with params " + x1 + " " + (x2 - 1));
        }
      ...
      if (backsector.ceilingheight != frontsector.ceilingheight
        ...
        if (DEBUG) {
          System.out.println(
              "Entering ClipSolidWallSegment window with params " + x1 + " " + (x2 - 1));
        }
  ...
  protected abstract class SegDrawer implements ISegDrawer {
     ...
     * fixed_t
     */
    protected int pixhigh, pixlow, pixhighstep, pixlowstep, topfrac, topstep, bottomfrac,
        bottomstep;
