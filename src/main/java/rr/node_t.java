...
public class node_t implements Resettable {
  ...
  private static final boolean OLDDEMO =
      Engine.getConfig().equals(Settings.line_of_sight, Settings.LOS.Vanilla);
  /**
   * Partition line.
   */
  @fixed_t
  ...
  public int DivlineSide(int x, int y) {
    int left, right;
    return (this.dx == 0) ? x == this.x ? 2 : x <= this.x ? eval(this.dy > 0) : eval(this.dy < 0) :
        (this.dy == 0)
            ?
            (OLDDEMO ? x : y) == this.y ? 2 : y <= this.y ? eval(this.dx < 0) : eval(this.dx > 0) :
            (this.dy == 0)
