...
public abstract class MaskedWorker<T, V> extends AbstractThings<T, V>
  ...
  public MaskedWorker(VideoScale vs, SceneRenderer<T, V> R, int id, int numthreads,
  ...
  protected final void DrawVisSprite(vissprite_t<V> vis) {
    ...
    if (maskedcvars.dc_colormap == null) {
      ...
      @SuppressWarnings("unchecked") final T translation =
    ...
    for (maskedcvars.dc_x = x1; maskedcvars.dc_x <= x2; maskedcvars.dc_x++, frac += vis.xiscale) {
      ...
      if (true) {
        if (texturecolumn < 0 || texturecolumn >= patch.width) {
          I.Error("R_DrawSpriteRange: bad texturecolumn %d vs %d %d %d", texturecolumn, patch.width,
              x1, x2);
        }
      }
  ...
  protected final void DrawPSprite(pspdef_t psp) {
    ...
    if (RANGECHECK) {
      if ((psp.state.frame & FF_FRAMEMASK) >= sprdef.numframes) {
        I.Error("R_ProjectSprite: invalid sprite frame %d : %d ", psp.state.sprite,
            psp.state.frame);
      }
    }
  ...
  public static final class HiColor extends MaskedWorker<byte[], short[]> {
    ...
    public HiColor(VideoScale vs, SceneRenderer<byte[], short[]> R, int id,
      ...
      colfuncshi.base = colfuncshi.main = colfuncshi.masked =
          new R_DrawColumnBoom.HiColor(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I);
      colfuncslow.masked =
          new R_DrawColumnBoomLow.HiColor(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I);

      // Fuzzy columns. These are also masked.
      colfuncshi.fuzz =
          new R_DrawFuzzColumn.HiColor(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I, BLURRY_MAP);
      colfuncslow.fuzz =
          new R_DrawFuzzColumnLow.HiColor(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I, BLURRY_MAP);

      // Translated columns are usually sprites-only.
      colfuncshi.trans =
          new R_DrawTranslatedColumn.HiColor(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I);
      colfuncslow.trans =
          new R_DrawTranslatedColumnLow.HiColor(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I);
  ...
  public static final class Indexed extends MaskedWorker<byte[], byte[]> {
    ...
    public Indexed(VideoScale vs, SceneRenderer<byte[], byte[]> R, int id,
      ...
      super(vs, R, id, numthreads, barrier);
      colfuncshi.base = colfuncshi.main = colfuncshi.masked =
          new R_DrawColumnBoom.Indexed(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I);
      colfuncslow.masked =
          new R_DrawColumnBoomLow.Indexed(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I);

      // Fuzzy columns. These are also masked.
      colfuncshi.fuzz =
          new R_DrawFuzzColumn.Indexed(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I, BLURRY_MAP);
      colfuncslow.fuzz =
          new R_DrawFuzzColumnLow.Indexed(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I, BLURRY_MAP);

      // Translated columns are usually sprites-only.
      colfuncshi.trans =
          new R_DrawTranslatedColumn.Indexed(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I);
      colfuncslow.trans =
          new R_DrawTranslatedColumnLow.Indexed(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I);
  ...
  public static final class TrueColor extends MaskedWorker<byte[], int[]> {
    ...
    public TrueColor(VideoScale vs, SceneRenderer<byte[], int[]> R, int id,
      ...
      // Non-optimized stuff for masked.
      colfuncshi.base = colfuncshi.main = colfuncshi.masked =
          new R_DrawColumnBoom.TrueColor(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I);
      colfuncslow.masked =
          new R_DrawColumnBoomLow.TrueColor(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I);

      // Fuzzy columns. These are also masked.
      colfuncshi.fuzz =
          new R_DrawFuzzColumn.TrueColor(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I, BLURRY_MAP);
      colfuncslow.fuzz =
          new R_DrawFuzzColumnLow.TrueColor(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I, BLURRY_MAP);

      // Translated columns are usually sprites-only.
      colfuncshi.trans =
          new R_DrawTranslatedColumn.TrueColor(vs.getScreenWidth(), vs.getScreenHeight(), ylookup,
              columnofs, maskedcvars, screen, I);
      colfuncslow.trans =
          new R_DrawTranslatedColumnLow.TrueColor(vs.getScreenWidth(), vs.getScreenHeight(),
              ylookup, columnofs, maskedcvars, screen, I);
