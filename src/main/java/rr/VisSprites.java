...
public final class VisSprites<V>
  ...
  private final RendererState<?, V> rendererState;
  private vissprite_t<V>[] vissprites;
  ...
  public void AddSprites(sector_t sec) {
    ...
    lightnum = (sec.lightlevel >> rendererState.colormaps.lightSegShift()) +
        rendererState.colormaps.extralight;
    ...
    if (lightnum < 0) {
      ...
      rendererState.colormaps.spritelights =
          rendererState.colormaps.scalelight[rendererState.colormaps.lightLevels() - 1];
    } else {
   ...
   * @param thing
   */
  private void ProjectSprite(mobj_t thing) {
    int tr_x, tr_y;
    int gxt, gyt;
    int tx, tz;
    ...
    if ((thing.flags & MF_SHADOW) != 0) {
      ...
      // diminished light
      index =
          xscale >> (rendererState.colormaps.lightScaleShift() - rendererState.view.detailshift);
   ...
   * @return
   */
  private vissprite_t<V> NewVisSprite() {
