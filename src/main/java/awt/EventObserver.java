...
public class EventObserver<Handler extends Enum<Handler> & EventBase<Handler>> {
   ...
   */
  protected final event_t.mouseevent_t mouseEvent =
   ...
   */
  public EventObserver(Class<Handler> handlerClass, Component component,
  ...
  public void observe(final AWTEvent ev) {
    ...
    if (!maybe.isPresent() ||
        !actionStateHolder.hasActionsEnabled(handler = maybe.get(), ActionMode.PERFORM)) {
      return;
    }
  ...
  protected void centreCursor(final AWTEvent event) {
    ...
    if (component.isShowing()) {
      MOUSE_ROBOT.ifPresent(
          rob -> mouseEvent.resetIn(rob, component.getLocationOnScreen(), centreX, centreY));
    }
    modifyCursor(event);
  }
  ...
  protected final void mapRelation(final Handler h, RelationType type, Handler... targets) {
    ...
    if (LOGGER.isLoggable(Level.FINE)) {
      LOGGER.log(Level.FINE, () -> String.format("RELATION MAPPING: %s -> [%s] {%s}", h, type,
  ...
  protected final void unmapRelation(final Handler h, RelationType type, Handler... targets) {
    ...
    if (LOGGER.isLoggable(Level.FINE)) {
      LOGGER.log(Level.FINE, () -> String.format("RELATION UNMAP: %s -> [%s] {%s}", h, type,
          Arrays.toString(targets)));
    }
  }
  ...
  protected final void restoreRelation(final Handler h, RelationType type, Handler... targets) {
    ...
    if (LOGGER.isLoggable(Level.FINE)) {
      LOGGER.log(Level.FINE, () -> String.format("RELATION RESTORE: %s -> [%s] {%s}", h, type,
          Arrays.toString(targets)));
    }
  }
