...
public interface EventBase<Handler extends Enum<Handler> & EventBase<Handler>> extends IntSupplier {
  ...
  final class KeyStateHolder<Handler extends Enum<Handler> & EventBase<Handler>> {
    ...
    public boolean notifyKeyChange(EventObserver<Handler> observer, Signals.ScanCode code,
                                   boolean press) {
      if (press) {
        holdingSet.add(code);
  ...
  final class ActionStateHolder<Handler extends Enum<Handler> & EventBase<Handler>> {
    ...
    private <V> Map<Handler, V> populate(Class<Handler> hClass, Handler[] values,
                                         Function<? super Handler, ? extends V> mapper) {
    ...
    public ActionStateHolder<Handler> run(final Handler h, final ActionMode mode,
                                          final AWTEvent ev) {
      if (enabledActions.get(h).contains(mode)) {
        Optional.ofNullable(actionsMap.get(h).get(mode))
            .ifPresent(action -> action.act(observer, ev));
      }
    ...
    @SafeVarargs
    public final ActionStateHolder<Handler> unmapCooperation(final Handler h, RelationType type,
                                                             final Handler... targets) {
    ...
    @SafeVarargs
    public final ActionStateHolder<Handler> mapCooperation(final Handler h, RelationType mode,
                                                           final Handler... targets) {
    ...
    @SafeVarargs
    public final ActionStateHolder<Handler> restoreCooperation(final Handler h, RelationType mode,
                                                               final Handler... targets) {
    ...
    @SafeVarargs
    public final ActionStateHolder<Handler> unmapAdjustment(final Handler h, RelationType type,
                                                            final Handler... targets) {
    ...
    @SafeVarargs
    public final ActionStateHolder<Handler> mapAdjustment(final Handler h, RelationType mode,
                                                          final Handler... targets) {
    ...
    @SafeVarargs
    public final ActionStateHolder<Handler> restoreAdjustment(final Handler h, RelationType mode,
                                                              final Handler... targets) {
      final Set<Handler> orig = h.adjustments().get(mode);
    ...
    public ActionStateHolder<Handler> mapAction(final Handler h, ActionMode mode,
                                                EventAction<Handler> remap) {
      actionsMap.get(h).put(mode, remap);

      return this;
    }

    public ActionStateHolder<Handler> remapAction(final Handler h, ActionMode mode,
                                                  EventAction<Handler> remap) {
