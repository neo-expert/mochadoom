...
public enum EventHandler implements EventBase<EventHandler> {
  ...
  RELATIONS(relationMapper -> {
    ...
    relationMapper.map(RelationType.ENABLE, Relate(WINDOW_OPEN,
        WINDOW_LOSE_FOCUS, KEY_PRESS, KEY_RELEASE, KEY_TYPE, MOUSE_ENTER, MOUSE_MOVE, MOUSE_DRAG,
    ...
    relationMapper.map(RelationType.DISABLE, Relate(WINDOW_LOSE_FOCUS, WINDOW_LOSE_FOCUS,
        KEY_PRESS, KEY_RELEASE, KEY_TYPE, MOUSE_MOVE, MOUSE_DRAG, MOUSE_PRESS, MOUSE_RELEASE,
        MOUSE_ENTER
    ));
     ...
     */
    relationMapper.map(RelationType.ENABLE,
        Relate(MOUSE_ENTER, MOUSE_MOVE, MOUSE_DRAG, MOUSE_PRESS, MOUSE_RELEASE));
  ...
  EventHandler(final Consumer<RelationMapper<EventHandler>> relationMapper) {
    this.eventId = -1;
  ...
  EventHandler(final int eventId, final ActionMode... enableModes) {
    this(eventId, null, enableModes);
  }
  EventHandler(final int eventId, final Consumer<ActionMapper<EventHandler>> actionMapper,
               final ActionMode... enableModes) {
  ...
  private static EventAction<EventHandler> mouseMoveAction(boolean isDrag) {
    return (observer, ev) -> {
      ...
      final int centreX = observer.component.getWidth() >> 1, centreY =
          observer.component.getHeight() >> 1;
      if (observer.component.isShowing() && EventObserver.MOUSE_ROBOT.isPresent()) {
        ...
        observer.mouseEvent.moveIn((MouseEvent) ev, EventObserver.MOUSE_ROBOT.get(), offset,
            centreX, centreY, isDrag);
      } else {
