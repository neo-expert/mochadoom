...
//  because of timing issues in netgames.
  ...
  protected void drawLF() {
    ...
    // draw <LevelName>
    DOOM.graphicSystem.DrawPatchScaled(FG, lnames[wbs.last], DOOM.vs,
        (320 - lnames[wbs.last].width) / 2, y);
  ...
  protected void drawEL() {
    ...
    DOOM.graphicSystem.DrawPatchScaled(FG, lnames[wbs.next], DOOM.vs,
        (320 - lnames[wbs.next].width) / 2, y);
  ...
  drawOnLnode(int n,
    ...
    if (fits && i < 2) {
      ...
      //	    FB, c[i]);
      DOOM.graphicSystem.DrawPatchScaled(FG, c[i], DOOM.vs, lnodes[wbs.epsd][n].x,
          lnodes[wbs.epsd][n].y);
    } else {
      // DEBUG
  ...
  protected int drawNum(int x, int y, int n, int digits) {
    ...
    if (neg) {
      DOOM.graphicSystem.DrawPatchScaled(FG, wiminus, DOOM.vs, x -= 8 * DOOM.vs.getScalingX(), y,
          V_NOSCALESTART);
    }
...
//
  protected void drawTime(int x,
    ...
    if (t <= 61 * 59) {
      ...
      // "sucks"
      DOOM.graphicSystem.DrawPatchScaled(FG, sucks, DOOM.vs,
          x - sucks.width * DOOM.vs.getScalingX(), y, V_NOSCALESTART);
    }
  }
  ...
  protected void drawDeathmatchStats() {
    ...
    DOOM.graphicSystem.DrawPatch(FG, total, DM_TOTALSX - total.width / 2,
  ...
  protected void drawNetgameStats() {
    ...
    // draw stat titles (top line)
    DOOM.graphicSystem.DrawPatchScaled(FG, kills, DOOM.vs, NG_STATSX() + NG_SPACINGX - kills.width,
        NG_STATSY);
    DOOM.graphicSystem.DrawPatchScaled(FG, items, DOOM.vs,
        NG_STATSX() + 2 * NG_SPACINGX - items.width, NG_STATSY);
    DOOM.graphicSystem.DrawPatchScaled(FG, secret, DOOM.vs,
        NG_STATSX() + 3 * NG_SPACINGX - secret.width, NG_STATSY);

    if (dofrags != 0) {
      DOOM.graphicSystem.DrawPatchScaled(FG, frags, DOOM.vs,
          NG_STATSX() + 4 * NG_SPACINGX - frags.width, NG_STATSY);
    }
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      ...
      x += NG_SPACINGX;
      drawPercent((x - pwidth) * DOOM.vs.getScalingX(), (y + 10) * DOOM.vs.getScalingY(),
          cnt_kills[i]);
      x += NG_SPACINGX;
      drawPercent((x - pwidth) * DOOM.vs.getScalingX(), (y + 10) * DOOM.vs.getScalingY(),
          cnt_items[i]);
  ...
  protected void drawStats() {
    ...
    DOOM.graphicSystem.DrawPatchScaled(FG, items, DOOM.vs, SP_STATSX, SP_STATSY + lh,
        V_NOSCALESTART);
    drawPercent(DOOM.vs.getScreenWidth() - SP_STATSX, SP_STATSY + lh, cnt_items[0]);

    DOOM.graphicSystem.DrawPatchScaled(FG, sp_secret, DOOM.vs, SP_STATSX, SP_STATSY + 2 * lh,
        V_NOSCALESTART);
    ...
    if (wbs.epsd < 3) {
      DOOM.graphicSystem.DrawPatchScaled(FG, par, DOOM.vs, DOOM.vs.getScreenWidth() / 2 + SP_TIMEX,
          SP_TIMEY, V_NOSCALESTART);
...
//System.out.println("State "+state);
  ...
  protected void loadData() {
    ...
    if (DOOM.isCommercial()) {
      ...
      // splat
      splat =
          new patch_t[] {DOOM.wadLoader.CacheLumpName("WISPLAT", PU_STATIC, patch_t.class), null};
