...
public interface Attacks extends Monsters {
  ...
  default void A_FireShotgun2(player_t player, pspdef_t psp) {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      angle += (long) (P_Random() - P_Random()) << 19;
      getAttacks().LineAttack(player.mo, angle, MISSILERANGE,
          sp.bulletslope + ((P_Random() - P_Random()) << 5), damage);
    }
  }
  ...
  default void A_Punch(player_t player, pspdef_t psp) {
    ...
    angle += (long) (P_Random() - P_Random()) << 18;
  ...
  default void A_FireCGun(player_t player, pspdef_t psp) {
    ...
    player.SetPsprite(ps_flash,
        statenum_t.values()[flashstate + current_state - statenum_t.S_CHAIN1.ordinal()]
    );
  ...
  default void A_BFGSpray(mobj_t mo) {
    ...
    for (int i = 0; i < 40; i++) {
      ...
      getEnemies().SpawnMobj(sp.linetarget.x, sp.linetarget.y,
          sp.linetarget.z + (sp.linetarget.height >> 2), mobjtype_t.MT_EXTRABFG);
