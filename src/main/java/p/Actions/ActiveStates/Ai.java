...
public interface Ai extends Monsters, Sounds {
  ...
  default void A_Chase(mobj_t actor) {
    ...
    if (actor.info.missilestate !=
        statenum_t.S_NULL) { //_D_: this caused a bug where Demon for example were disappearing
      // Assume that a missile attack is possible
      if (getGameSkill().ordinal() < skill_t.sk_nightmare.ordinal() && !IsFastParm() &&
          actor.movecount != 0) {
        // Uhm....no.
        nomissile = true;
  ...
  default void A_FaceTarget(mobj_t actor) {
    ...
    if (eval(actor.target.flags & MF_SHADOW)) {
      actor.angle += (long) (P_Random() - P_Random()) << 21;
    }
    actor.angle &= BITS32;
  }
}
