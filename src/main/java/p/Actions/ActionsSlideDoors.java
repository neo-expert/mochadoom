...
public interface ActionsSlideDoors extends ActionTrait {
  ...
  ContextKey<SlideDoors> KEY_SLIDEDOORS =
      ACTION_KEY_CHAIN.newKey(ActionsSlideDoors.class, SlideDoors::new);
  // UNUSED
      ...
      case sd_opening:
        if (door.timer-- == 0) {
          if (++door.frame == ActionsSlideDoors.SNUMFRAMES) {
            ...
            ll.sides[door.line.sidenum[0]].midtexture =
                (short) sd.slideFrames[door.whichDoorIndex].frontFrames[door.frame];
            ll.sides[door.line.sidenum[1]].midtexture =
                (short) sd.slideFrames[door.whichDoorIndex].backFrames[door.frame];
          }
        }
        break;
