...
public interface ActionsSpawns extends ActionsSectors {
  ...
  default void SpawnPlayer(mapthing_t mthing) {
    ...
    if (mthing.type > 1) {
      mobj.flags |= (long) (mthing.type - 1) << MF_TRANSSHIFT;
    }
  ...
  default mobj_t SpawnMapThing(mapthing_t mthing) {
    ...
    if (i == NUMMOBJTYPES) {
      Spawn.LOGGER.log(Level.WARNING,
          String.format("P_SpawnMapThing: Unknown type %d at (%d, %d)", mthing.type, mthing.x,
              mthing.y));
      return null;
    }
    ...
    if (D.nomonsters &&
        (i == mobjtype_t.MT_SKULL.ordinal() || eval(mobjinfo[i].flags & MF_COUNTKILL))) {
      return null;
    }
