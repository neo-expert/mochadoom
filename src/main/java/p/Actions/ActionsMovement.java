...
public interface ActionsMovement extends ActionsPathTraverse {
  ...
  default boolean TryMove(mobj_t thing, @fixed_t int x, @fixed_t int y) {
    ...
    if (!eval(thing.flags & MF_NOCLIP)) {
      ...
      if (!eval(thing.flags & (MF_DROPOFF | MF_FLOAT)) &&
          mov.tmfloorz - mov.tmdropoffz > 24 * FRACUNIT) {
  ...
  default void SlideMove(mobj_t mo) {
    ...
    do {
      ...
      PathTraverse(leadx, leady, leadx + mo.momx, leady + mo.momy, PT_ADDLINES,
          this::SlideTraverse);
      PathTraverse(trailx, leady, trailx + mo.momx, leady + mo.momy, PT_ADDLINES,
          this::SlideTraverse);
      PathTraverse(leadx, traily, leadx + mo.momx, traily + mo.momy, PT_ADDLINES,
          this::SlideTraverse);
