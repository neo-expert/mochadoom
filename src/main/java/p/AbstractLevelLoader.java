...
public abstract class AbstractLevelLoader implements ILevelLoader {
  ...
  public static final boolean FIX_BLOCKMAP_512 =
      Engine.getConfig().equals(Settings.fix_blockmap, Boolean.TRUE);
  /**
   ...
   * @param lineno
   */
  private void AddBlockLine(linelist_t[] lists, int[] count, boolean[] done, int blockno,
                            int lineno) {
      ...
      for (x = 0; x < bmapwidth; x++) {
        ...
        for (tmplist = p_list; ; tmplist++) {
          ...
          if (tmplist >= p_maxoffs) {
            System.err.print("P_VerifyBlockMap: open blocklist\n");
            return false;
          }
        ...
        for (tmplist = p_list; blockmaplump[tmplist] != -1; tmplist++) {
          if (blockmaplump[tmplist] < 0 || blockmaplump[tmplist] >= numlines) {
            System.err.print("P_VerifyBlockMap: index >= numlines\n");
            return false;
          }
        }
      }
