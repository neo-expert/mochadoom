...
public class WadLoader implements IWadLoader {
   ...
   */
  private final List<wadfile_info_t> wadfiles;
  ...
  private void AddFile(String uri, ZipEntry entry, int type) throws IOException {
    ...
    if (!C2JUtils.checkForExtension(checkname, "wad")) {
      ...
      handle = InputStreamSugar.streamSeek(handle, header.infotableofs, wadinfo.maxsize, uri, entry,
          type);
  ...
  @Override
  public void InitMultipleFiles(String[] filenames) throws IOException {
    int size;
  ...
  public <T extends CacheableDoomObject> T[] CacheLumpNumIntoArray(int lump, int num,
                                                                   ArraySupplier<T> what,
                                                                   IntFunction<T[]> arrGen) {
    if (lump >= numlumps) {
    ...
    @SuppressWarnings("unchecked") final CacheableDoomObjectContainer<T> cont =
        (CacheableDoomObjectContainer<T>) lumpcache[lump];
