...
public class CVarManager {
   ...
   * @return Optional
   */
  public <T> Optional<T> get(final CommandVariable cv, final Class<T> itemType,
                             final int position) {
    if (cv.arguments[position] == itemType) {
      ...
      @SuppressWarnings("unchecked") final T ret = (T) cVarMap.get(cv)[position];
      return Optional.ofNullable(ret);
    }

    throw new IllegalArgumentException(
        "CVar argument at position " + position + " is not of class " + itemType.getName());
  }
  ...
  public <T> boolean with(final CommandVariable cv, final int position, final Consumer<T> action) {
    try {
      @SuppressWarnings("unchecked") final Object[] mapped = cVarMap.get(cv);
      if (mapped == null) {
        return false;
      }

      @SuppressWarnings("unchecked") final T item = (T) mapped[position];
      action.accept(item);
      return true;
  ...
  private void readResponseFile(final String filename) {
    ...
    if (new ResourceIO(filename).readLines(r)) {
      System.out.printf("Found response file %s, read %d command line variables%n", filename,
          r.cVarCount);
    } else {
      System.out.printf("No such response file %s!%n", filename);
      System.exit(1);
    }
  }
  ...
  private int processCVarSubArgs(final List<String> commandList, int position,
                                 final CommandVariable cVar) {
    ...
    for (int j = 0; j < cVar.arguments.length; ++j) {
      ...
      } else if ((cVarMappings[j] = processValue(cVar.arguments[j], commandList, position + 1)) ==
          null) {
        break;
      } else {
        ++position;
  ...
  private Object processValue(final Class<?> elementClass, final List<String> commandList,
                              int position) {
  ...
  private Object[] processVarArg(final Class<?> elementClass, final List<String> commandList,
                                 int position) {
    final List<Object> list = new ArrayList<>();
    for (Object value; (value = processValue(elementClass, commandList, position)) != null;
         ++position) {
      list.add(value);
    }
