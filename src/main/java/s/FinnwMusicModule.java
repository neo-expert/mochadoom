...
public class FinnwMusicModule implements IMusic {
  ...
  EventGroup nextEventGroup(ByteBuffer scoreBuffer, boolean looping) {
    ...
    do {
      ...
      switch (eventType) {
        ...
        default:
          String msg =
              String.format("Unknown event type: last=%5s eventType=%d chanIndex=%d%n", last,
                  eventType, chanIndex);
          throw new IllegalArgumentException(msg);
      }
    } while (!last);
  ...
  static class EventGroup {
    ...
    void pitchBendSensitivity(int midiChan, int semitones) {
      addRegParamChange(midiChan, RPM_PITCH_BEND_SENSITIVITY, RPL_PITCH_BEND_SENSITIVITY,
          semitones);
    }
    ...
    Song(ByteBuffer data) {
      ...
      if (!hasMusMagic(magicBuf)) {
        throw new IllegalArgumentException(
            "Expected magic string \"MUS\\x1a\" but found " + Arrays.toString(magic));
      }
