...
public class DSP {
   ...
   * parameters:
   *
   * @param fsr = 44100 : rem set fsr = sample rate
   * @param fc  = 0 : rem set fc = 0 for lowpass fc = center frequency for
   *            bandpass filter fc = fsr/2 for a highpass
   * @param bw  = 3000 : rem bw = bandwidth, range 0 .. fsr/2 and bw >= fsr/n bw =
   *            3 db corner frequency for a lowpass bw = half the 3 db passband
   *            for a bandpass filter
   * @param nt  = 128 : rem nt = number of taps + 1 (nt must be even) nt should be
   *            > fsr / bw transition band will be around 3.5 * fsr / nt depending
   *            on the window applied and ripple spec.
   * @param g   = 1 : rem g = filter gain for bandpass g = 0.5 , half the gain for
   *            a lowpass filter
  ...
  private static void getLPCoefficientsButterworth2Pole(final int samplerate, final double cutoff,
                                                        final double[] ax, final double[] by) {
   ...
   * @param wnwdth = width of windowed Sinc used as the low pass filter rem resamp()
