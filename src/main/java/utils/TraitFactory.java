...
public class TraitFactory {
      ...
      for (final Field f : declaredFields) {
        ...
        if (Modifier.isPublic(modifiers) && Modifier.isStatic(modifiers) &&
          ...
          if (fieldClass == ContextKey.class) {
            final ContextKey<?> key = (ContextKey) f.get(null);
            c.put(key, key.contextConstructor);
            LOGGER.fine(
                () -> String.format("%s for %s", c.get(key).getClass(), f.getDeclaringClass()));
          }
        }
      }
  ...
  public interface Trait {
    ...
    default <T, E extends Throwable> T contextRequire(ContextKey<T> key,
  ...
  public final static class ContextKey<T> {
    ...
    public ContextKey(final Class<? extends Trait> traitClass, int preferredId,
                      Supplier<T> contextConstructor) {
  ...
  public final static class KeyChain {
    ...
    public <T> ContextKey<T> newKey(final Class<? extends Trait> traitClass,
                                    Supplier<T> contextConstructor) {
