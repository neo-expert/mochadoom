...
abstract class SoftwareParallelVideoRenderer<T, V> extends SoftwareGraphicsSystem<T, V> {
  protected static final int PARALLELISM =
      Engine.getConfig().getValue(Settings.parallelism_realcolor_tint, Integer.class);
  protected static final GraphicsConfiguration GRAPHICS_CONF =
  ...
  static boolean checkConfigurationHicolor() {
    ...
    return cps == 3 && cm.getComponentSize(0) == 5 && cm.getComponentSize(1) == 5 &&
        cm.getComponentSize(2) == 5;
  }
  ...
  static boolean checkConfigurationTruecolor() {
    final ColorModel cm = GRAPHICS_CONF.getColorModel();
    final int cps = cm.getNumComponents();
    return cps == 3 && cm.getComponentSize(0) == 8 && cm.getComponentSize(1) == 8 &&
        cm.getComponentSize(2) == 8;
  }
  ...
  public V convertPalettedBlock(byte... data) {
    ...
    return (V) (isShort ? new short[] {(short) getBaseColor(data[0])} :
        new int[] {getBaseColor(data[0])});
  }
}
