...
class BufferedRenderer extends SoftwareIndexedVideoRenderer {
  ...
  BufferedRenderer(RendererFactory.WithWadLoader<byte[], byte[]> rf) {
    ...
    for (DoomScreen s : DoomScreen.values()) {
      ...
      rasters[index] = Raster.createInterleavedRaster(db, width, height, width, 1, new int[] {0},
          new Point(0, 0));
    }
  ...
  public final void forcePalette() {
    this.currentscreen =
        new BufferedImage(cmaps[usegamma][usepalette], rasters[DoomScreen.FG.ordinal()], true,
            null);
  }
}
