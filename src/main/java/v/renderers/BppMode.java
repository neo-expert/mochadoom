...
public enum BppMode {
  ...
  <T, V> BppMode(int lightBits, RenderGen<T, V> renderGen, ScenerGen<T, V> scenerGen,
                 int transparency) {
  ...
  private static <T, V> SceneRenderer<T, V> SceneGen_8(DoomMain<T, V> DOOM) {
    return (SceneRenderer<T, V>) SceneRendererMode.getMode().indexedGen.apply(
        (DoomMain<byte[], byte[]>) DOOM);
  }

  @SuppressWarnings("unchecked")
  private static <T, V> SceneRenderer<T, V> SceneGen_16(DoomMain<T, V> DOOM) {
    return (SceneRenderer<T, V>) SceneRendererMode.getMode().hicolorGen.apply(
        (DoomMain<byte[], short[]>) DOOM);
  }

  @SuppressWarnings("unchecked")
  private static <T, V> SceneRenderer<T, V> SceneGen_32(DoomMain<T, V> DOOM) {
    return (SceneRenderer<T, V>) SceneRendererMode.getMode().truecolorGen.apply(
        (DoomMain<byte[], int[]>) DOOM);
  }
