...
public class BlurryTable implements FuzzMix, Colors {
  ...
  private final boolean semiTranslucent =
      Engine.getConfig().equals(Settings.semi_translucent_fuzz, Boolean.TRUE);
  ...
  public BlurryTable(short[][] liteColorMaps) {
    ...
    for (int i = 0; i < PAL_NUM_COLORS; ++i) {
      ...
      final short avgColor =
          GreyscaleFilter.grey555(blurryColor[0], blurryColor[1], blurryColor[2]);
      ...
      final float bestRatio = GreyscaleFilter.component(ratioR, ratioG,
          ratioB);//ratioR * ratioR * ratioG * ratioB * ratioB;
