...
public interface DoomGraphicSystem<T, V> {
   ...
   */
  int V_NOSCALESTART = 0x00010000;   // dont scale x,y, start coords
  int V_SCALESTART = 0x00020000;   // scale x,y, start coords
  int V_SCALEPATCH = 0x00040000;   // scale patch
  int V_NOSCALEPATCH = 0x00080000;   // don't scale patch
  int V_WHITEMAP = 0x00100000;   // draw white (for v_drawstring)
  int V_FLIPPEDPATCH = 0x00200000;   // flipped in y
  int V_TRANSLUCENTPATCH = 0x00400000;   // draw patch translucent
  int V_PREDIVIDE = 0x00800000;   // pre-divide by best x/y scale.
  int V_SCALEOFFSET = 0x01000000;   // Scale the patch offset
  int V_NOSCALEOFFSET = 0x02000000;   // dont's cale patch offset
  int V_SAFESCALE = 0x04000000;   // scale only by minimal scale of x/y instead of both
  ...
  void DrawPatchCenteredScaled(DoomScreen screen, patch_t patch, VideoScale vs, int y,
                               int... flags);
