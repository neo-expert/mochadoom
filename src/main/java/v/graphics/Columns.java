...
public interface Columns<V, E extends Enum<E>> extends Blocks<V, E> {
   ...
   */
  default void DrawPatchColumns(V screen, patch_t patch, int x, int y, int dupx, int dupy,
                                boolean flip) {
  ...
  class U {
    static final int COLUMN_THREADS =
        Engine.getConfig().getValue(Settings.parallelism_patch_columns, Integer.class);
    private static final ForkJoinPool pool =
        COLUMN_THREADS > 0 ? new ForkJoinPool(COLUMN_THREADS) : null;
