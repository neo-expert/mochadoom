...
public interface Palettes extends Lights {
   ...
   */
  default void ColorShiftPalette(byte[] inpal, byte[] outpal, int r, int g, int b, int shift,
                                 int steps) {
    int in_p = 0;
    int out_p = 0;
    for (int i = 0; i < PAL_NUM_COLORS; i++) {
      final int dr = r - inpal[in_p];
      final int dg = g - inpal[in_p + 1];
      final int db = b - inpal[in_p + 2];
      outpal[out_p] = (byte) (inpal[in_p] + dr * shift / steps);
