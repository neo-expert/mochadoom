...
public interface Patches<V, E extends Enum<E>> extends Columns<V, E> {
   ...
   * parsed afterwards. Shamelessly ripped from Doom Legacy (for menus, etc) by _D_ ;-)
   */
  default void DrawPatchCenteredScaled(E screen, patch_t patch, VideoScale vs, int y,
                                       int... flags) {
    ...
    final boolean scaleOffset =
        !C2JUtils.flags(flagsV, V_NOSCALEOFFSET) || C2JUtils.flags(flagsV, V_SCALEOFFSET);
    // By default we scale, if V_NOSCALESTART we dont scale unless V_SCALESTART (restores Default Behavior)
    final boolean scaleStart =
        !C2JUtils.flags(flagsV, V_NOSCALESTART) || C2JUtils.flags(flagsV, V_SCALESTART);
    // By default we do dup, if V_NOSCALEPATCH we dont dup unless V_SCALEPATCH (restores Default Behavior)
    final boolean noScalePatch =
        C2JUtils.flags(flagsV, V_NOSCALEPATCH) && !C2JUtils.flags(flagsV, V_SCALEPATCH);
    final boolean flip = C2JUtils.flags(flagsV, V_FLIPPEDPATCH);
    final int halfWidth = noScalePatch ? patch.width / 2 : patch.width * dupx / 2;
    int x = getScreenWidth() / 2 - halfWidth -
        (scaleOffset ? patch.leftoffset * dupx : patch.leftoffset);
   ...
   */
  default void printDebugPatchInfo(patch_t patch, int x, int y, final boolean predevide,
                                   final boolean scaleOffset, final boolean scaleStart, int dupx,
                                   int dupy) {
  ...
  default int applyScaling(int c, int offset, int dup, boolean predevide, boolean scaleOffset,
                           boolean scaleStart) {
