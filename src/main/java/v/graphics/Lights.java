...
public interface Lights extends Colors {
   ...
   */
  float LIGHT_INCREMENT_RATIO_24 = 1.0f / COLORMAP_LIGHTS_24;
    ...
    for (int i = 1; i < COLORMAP_LIGHTS_24; ++i) {
      ...
      for (int j = 0; j < PAL_NUM_COLORS; ++j) {
        ...
        targetColormap[i][j] = toRGB888(
            sigmoidGradient(color1[0],
                (int) (Math.max(color2[0], color0[0]) * currentLightRatio + 0.5), gradient),
            sigmoidGradient(color1[1],
                (int) (Math.max(color2[1], color0[1]) * currentLightRatio + 0.5), gradient),
            sigmoidGradient(color1[2],
                (int) (Math.max(color2[2], color0[2]) * currentLightRatio + 0.5), gradient)
        );
      }

      // now detect if we are lightening or darkening
      currentLightRatio +=
          weight > PAL_NUM_COLORS ? LIGHT_INCREMENT_RATIO_24 : -LIGHT_INCREMENT_RATIO_24;
    }
  ...
  default short[][] BuildLights15(int[] palette, byte[][] colormaps) {
    final short[][] targetColormap =
        new short[Math.max(colormaps.length, COLORMAP_STD_LENGTH_15)][PAL_NUM_COLORS];
