...
public interface Melt extends ColorTransform {
  ...
  default void toScreen(Class<?> bufType, Object src, Object dest, int width, int dy, int ps,
                        int pd) {
    ...
    while (wiper.ticks-- > 0) {
      for (int i = 0; i < lim; i++) {
        ...
        if (wiper.y[i] < 0) {
          ...
          int dy = (wiper.y[i] < wiper.scaled_16) ? wiper.y[i] + (scaled ? wiper.dupy : 1) :
              wiper.scaled_8;
          ...
          if (scaled) {
            ...
            toScreen(wiper.bufferType, wiper.wipeScr, wiper.wipeEndScr, wiper.screenWidth, dy, pd,
                pd);
          }
          ...
          if (scaled) {
            toScreenScaled(wiper, wiper.wipeStartScr, wiper.screenHeight - wiper.y[i],
                i * wiper.dupy, pd);
          } else {
            toScreen(wiper.bufferType, wiper.wipeScr, wiper.wipeStartScr, wiper.screenWidth,
                wiper.screenHeight - wiper.y[i], i, pd);
          }
