...
import static data.Defines.PLAYERRADIUS;
...
import static doom.englsh.AMSTR_FOLLOWON;
import static doom.englsh.AMSTR_GRIDOFF;
...
public class Map<T, V> implements IAutoMap<T, V> {
  ...
  public final void Repalette() {
    ...
    Arrays.stream(Color.values())
        .forEach((c) -> {
          ...
          V extended =
              (V) Array.newInstance(converted.getClass().getComponentType(), Color.NUM_LITES);
  ...
  public final void initVariables() {
    ...
    if (!DOOM.players[pnum = DOOM.consoleplayer].inGame) {
      for (pnum = 0; pnum < MAXPLAYERS; pnum++) {
        System.out.println(pnum);
        if (DOOM.players[pnum].inGame) {
          break;
        }
      }
    }
    ...
    for (int i = 0; i < DOOM.levelLoader.numlines; i++) {
      ...
      if ((cheating | (DOOM.levelLoader.lines[i].flags & ML_MAPPED)) != 0) {
        ...
        if (DOOM.levelLoader.lines[i].backsector == null) {
          ...
          } else if (DOOM.levelLoader.lines[i].backsector.floorheight !=
              DOOM.levelLoader.lines[i].frontsector.floorheight) {
            drawMline(l, fdWallColorSource); // floor level change
          } else if (DOOM.levelLoader.lines[i].backsector.ceilingheight !=
              DOOM.levelLoader.lines[i].frontsector.ceilingheight) {
  ...
  public final void drawPlayers() {
    ...
    for (int i = 0; i < MAXPLAYERS; i++) {
      ...
      drawLineCharacter(player_arrow, NUMPLYRLINES, 0, (int) p.mo.angle, colorSource, p.mo.x,
          p.mo.y);
    }
