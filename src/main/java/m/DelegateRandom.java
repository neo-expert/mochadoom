...
public class DelegateRandom implements IRandom {
  ...
  public void requireRandom(final int version) {
    if (C2JUtils.flags(version, Defines.JAVARANDOM_MASK) && this.random instanceof DoomRandom) {
      switchRandom(true);
    } else if (!C2JUtils.flags(version, Defines.JAVARANDOM_MASK) &&
        !(this.random instanceof DoomRandom)) {
      switchRandom(false);
    }
  }
  ...
  private void switchRandom(boolean which) {
    ...
    if (arandom != null &&
        ((!which && arandom instanceof DoomRandom) || (which && arandom instanceof JavaRandom))) {
      this.altRandom = random;
      this.random = arandom;
      System.out.printf("M_Random: Switching to %s\n", random.getClass().getSimpleName());
    } else {
      this.altRandom = random;
      this.random = which ? new JavaRandom() : new DoomRandom();
      System.out.printf("M_Random: Switching to %s (new instance)\n",
          random.getClass().getSimpleName());
    }
    //random.ClearRandom();
  }
