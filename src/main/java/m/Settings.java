...
public enum Settings {
  ...
  vanilla_key_behavior(FILE_MOCHADOOM,
      true), // Currently forces LSHIFT on RSHIFT, TODO: layouts, etc
  automap_plotter_style(FILE_MOCHADOOM,
      Plotter.Style.Thin), // Thin is vanilla, Thick is scaled, Deep slightly rounded scaled
  enable_colormap_lump(FILE_MOCHADOOM,
      true), // Enables usage of COLORMAP lump read from wad during lights and specials generation
  color_depth(FILE_MOCHADOOM,
      BppMode.Indexed), // Indexed: 256, HiColor: 32 768, TrueColor: 16 777 216
  extend_plats_limit(FILE_MOCHADOOM, true), // Resize instead of "P_AddActivePlat: no more plats!"
  extend_button_slots_limit(FILE_MOCHADOOM,
      true), // Resize instead of "P_StartButton: no button slots left!"
  fix_blockmap(FILE_MOCHADOOM, true), // Add support for 512x512 blockmap
  fix_gamma_ramp(FILE_MOCHADOOM,
      false), // Vanilla do not use pure black color because Gamma LUT calculated without it, doubling 128
  fix_gamma_palette(FILE_MOCHADOOM,
      false), // In vanilla, switching gamma with F11 hides Berserk or Rad suit tint
  fix_sky_change(FILE_MOCHADOOM,
      false), // In vanilla, sky does not change when you exit the level and the next level with new sky
  fix_sky_palette(FILE_MOCHADOOM,
      false), // In vanilla, sky color does not change when under effect of Invulnerability powerup
  fix_medi_need(FILE_MOCHADOOM,
      false), // In vanilla, message "Picked up a medikit that you REALLY need!" never appears due to bug
  fix_ouch_face(FILE_MOCHADOOM,
      false), // In vanilla, ouch face displayed only when acuired 25+ health when damaged for 25+ health
  line_of_sight(FILE_MOCHADOOM,
      LOS.Vanilla), // Deaf monsters when thing pos corellates somehow with map vertex, change desync demos
  ...
  scale_melt(FILE_MOCHADOOM,
      true), // If you scale melt and use DoomRandom generator (not truly random), it looks exacly like vanilla
  semi_translucent_fuzz(FILE_MOCHADOOM,
      false), // only works in AlphaTrueColor mode. Also ignored with fuzz_mix = true
  fuzz_mix(FILE_MOCHADOOM,
      false), // Maes unique features on Fuzz effect. Vanilla dont have that, so they are switched off by default
  parallelism_realcolor_tint(FILE_MOCHADOOM,
      Runtime.getRuntime().availableProcessors()), // Used for real color tinting to speed up
  parallelism_patch_columns(FILE_MOCHADOOM,
      0), // When drawing screen graphics patches, this speeds up column drawing, <= 0 is serial
  greyscale_filter(FILE_MOCHADOOM,
      GreyscaleFilter.Luminance), // Used for FUZZ effect or with -greypal comand line argument (for test)
  scene_renderer_mode(FILE_MOCHADOOM,
      SceneRendererMode.Serial), // In vanilla, scene renderer is serial. Parallel can be faster
  reconstruct_savegame_pointers(FILE_MOCHADOOM,
      true); // In vanilla, infighting targets are not restored on savegame load
