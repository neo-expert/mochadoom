...
import static g.Signals.ScanCode.SC_1;
import static g.Signals.ScanCode.SC_2;
import static g.Signals.ScanCode.SC_3;
import static g.Signals.ScanCode.SC_4;
import static g.Signals.ScanCode.SC_5;
import static g.Signals.ScanCode.SC_6;
import static g.Signals.ScanCode.SC_BACKSPACE;
...
import static g.Signals.ScanCode.SC_F1;
import static g.Signals.ScanCode.SC_F5;
import static g.Signals.ScanCode.SC_G;
import static g.Signals.ScanCode.SC_H;
import static g.Signals.ScanCode.SC_I;
import static g.Signals.ScanCode.SC_K;
import static g.Signals.ScanCode.SC_L;
import static g.Signals.ScanCode.SC_LEFT;
import static g.Signals.ScanCode.SC_M;
import static g.Signals.ScanCode.SC_N;
import static g.Signals.ScanCode.SC_NULL;
import static g.Signals.ScanCode.SC_O;
import static g.Signals.ScanCode.SC_PRTSCRN;
import static g.Signals.ScanCode.SC_Q;
import static g.Signals.ScanCode.SC_R;
import static g.Signals.ScanCode.SC_RIGHT;
import static g.Signals.ScanCode.SC_S;
import static g.Signals.ScanCode.SC_SPACE;
import static g.Signals.ScanCode.SC_T;
import static g.Signals.ScanCode.SC_U;
import static g.Signals.ScanCode.SC_UP;
import static g.Signals.ScanCode.SC_Y;
...
public class Menu<T, V> extends AbstractDoomMenu<T, V> {
   ...
   * too long
   */
  private static final String[] skullName = {"M_SKULL1", "M_SKULL2"};
  /**
   ...
   */
  menuitem_t[] MainMenu, EpisodeMenu, NewGameMenu, OptionsMenu, ReadMenu1, ReadMenu2, SoundMenu,
      LoadMenu, SaveMenu;
  /**
  ...
  private void DrawSaveLoadBorder(int x, int y) {
    ...
    DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_LSLEFT"), DOOM.vs,
        x - 8, y + 7);

    for (i = 0; i < 24; i++) {
      DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_LSCNTR"), DOOM.vs, x,
          y + 7);
      x += 8;
    }

    DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_LSRGHT"), DOOM.vs, x,
        y + 7);
  }
  ...
  public void DrawThermo(int x, int y, int thermWidth, int thermDot) {
    int xx = x;
    DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_THERML"), DOOM.vs, xx,
        y);
    xx += 8;
    for (int i = 0; i < thermWidth; i++) {
      DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_THERMM"), DOOM.vs, xx,
          y);
      xx += 8;
    }
    DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_THERMR"), DOOM.vs, xx,
        y);
    DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_THERMO"), DOOM.vs,
        (x + 8) + thermDot * 8, y);
  }

  public void DrawEmptyCell(menu_t menu, int item) {
    DOOM.graphicSystem.DrawPatchScaled(FG,
        DOOM.wadLoader.CacheLumpName("M_CELL1", PU_CACHE, patch_t.class), DOOM.vs, menu.x - 10,
  ...
  public void DrawSelCell(menu_t menu, int item) {
    DOOM.graphicSystem.DrawPatchScaled(FG,
        DOOM.wadLoader.CacheLumpName("M_CELL2", PU_CACHE, patch_t.class), DOOM.vs, menu.x - 10,
        menu.y + item * LINEHEIGHT - 1);
  }
  ...
  private void QuickSave() {
    ...
    tempstring =
        String.format(QSPROMPT, C2JUtils.nullTerminatedString(savegamestrings[quickSaveSlot]));
  ...
  public void QuickLoad() {
    ...
    tempstring =
        String.format(QLPROMPT, C2JUtils.nullTerminatedString(savegamestrings[quickSaveSlot]));
  ...
  public boolean Responder(event_t ev) {
    ...
    if (messageToPrint) {
      if (messageNeedsInput &&
          !(sc == SC_SPACE || sc == SC_N || sc == SC_Y || sc == SC_ESCAPE)) {
        return false;
      }
  ...
  public class M_DrawSave implements DrawRoutine {
    ...
    public void invoke() {
      int i;
      DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_SAVEG"), DOOM.vs, 72,
          28);
      for (i = 0; i < load_end; i++) {
  ...
  class M_DrawEpisode
    ...
    @Override
    public void invoke() {
      DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_EPISOD"), DOOM.vs, 54,
          38);
    }
  ...
  class M_DrawLoad
    ...
    public void invoke() {
      ...
      DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_LOADG"), DOOM.vs, 72,
          28);
      for (i = 0; i < load_end; i++) {
        DrawSaveLoadBorder(LoadDef.x, LoadDef.y + LINEHEIGHT * i);
  ...
  class M_DrawMainMenu
    ...
    @Override
    public void invoke() {
      DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_DOOM"), DOOM.vs, 94,
          2);
    }
  }
  ...
  class M_DrawNewGame
    ...
    @Override
    public void invoke() {
      DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_NEWG"), DOOM.vs, 96,
          14);
      DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_SKILL"), DOOM.vs, 54,
          38);
    }
  }
  ...
  class M_DrawOptions
    ...
    @Override
    public void invoke() {
      DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName("M_OPTTTL"), DOOM.vs,
          108, 15);
      DOOM.graphicSystem.DrawPatchScaled(FG,
          DOOM.wadLoader.CachePatchName(detailNames[detailLevel]), DOOM.vs, OptionsDef.x + 175,
          OptionsDef.y + LINEHEIGHT * detail);
      DOOM.graphicSystem.DrawPatchScaled(FG,
          DOOM.wadLoader.CachePatchName(msgNames[showMessages ? 1 : 0]), DOOM.vs,
  ...
  class M_DrawReadThis1 implements DrawRoutine {
    ...
    public void invoke() {
      ...
      ReadDef1.x = skullx;
      ReadDef1.y = skully;
      DOOM.graphicSystem.DrawPatchScaled(FG, DOOM.wadLoader.CachePatchName(lumpname), DOOM.vs, 0,
          0);
  ...
  class M_DrawSound
    ...
    public void invoke() {
      DOOM.graphicSystem.DrawPatchScaled(FG,
          DOOM.wadLoader.CacheLumpName("M_SVOL", PU_CACHE, patch_t.class), DOOM.vs, 60, 38);

      DrawThermo(SoundDef.x, SoundDef.y + LINEHEIGHT, 16,
          DOOM.snd_SfxVolume);
  ...
  class M_ChangeDetail implements MenuRoutine {
    ...
    public void invoke(int choice) {
      ...
      DOOM.sceneRenderer.SetViewSize(screenblocks, detailLevel);
      if (detailLevel == 0) {
        DOOM.players[DOOM.consoleplayer].message = englsh.DETAILHI;
      } else {
